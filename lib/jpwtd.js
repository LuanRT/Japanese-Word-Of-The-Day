...
async function getJapaneseWordOfTheDay() {
  ...
  const word =
  $($(selectors.japanese_wtd_selector)[0])
  .text()
  .trim();

  const kana =
  $($(selectors.japanese_wtd_kana_selector)[0])
  .text()
  .trim();

  const romaji =
  $($(selectors.japanese_wtd_romaji_selector)[0])
  .text()
  .trim();
  ...
  const example_english = $($(selectors.japanese_wtd_example_english_selector)[0])
  .text()
  .trim
